...
func main() {
	...
	verbose("Machine:"+fmt.Sprintf("%X", uint64(file.FileHeader.Machine)), "")
	verbose("Magic:"+fmt.Sprintf("%X", uint64(opt.Magic)), "")
	verbose("Subsystem:"+fmt.Sprintf("%X", uint64(opt.Subsystem)), "")
	if opt.CheckSum != 0x00 {
		verbose("Checksum:"+fmt.Sprintf("%X", uint64(opt.CheckSum)), "")
	}
	verbose("Image Base:"+fmt.Sprintf("%X", uint64(opt.ImageBase)), "")
	verbose("Address Of Entry:"+fmt.Sprintf("%X", uint64(opt.AddressOfEntryPoint)), "")
	verbose("Size Of Headers:"+fmt.Sprintf("%X", uint64(opt.SizeOfHeaders)), "")
	verbose("Size Of Image:"+fmt.Sprintf("%X", uint64(opt.SizeOfImage)), "")
	verbose("Export Table:"+fmt.Sprintf("%X", uint64(opt.DataDirectory[0].VirtualAddress)+opt.ImageBase), "")
	verbose("Import Table:"+fmt.Sprintf("%X", uint64(opt.DataDirectory[1].VirtualAddress)+opt.ImageBase), "")
	verbose("Base Relocation Table:"+fmt.Sprintf("%X", uint64(opt.DataDirectory[5].VirtualAddress)+opt.ImageBase), "")
	verbose("Import Address Table:"+fmt.Sprintf("%X", uint64(opt.DataDirectory[12].VirtualAddress)+opt.ImageBase), "")
	verbose("[-------------------------------------]\n", "")
